...
if CoreGui:FindFirstChild("Shaman") then
    ...
    local tooltips = CoreGui:FindFirstChild("Tooltips")
    if tooltips then
        tooltips:Destroy()
    end
end
...
local function ClampWindowPosition(frame)
    ...
    local x = math.clamp(frame.Position.X.Offset, 0, math.max(0, viewport.X - size.X))
    local y = math.clamp(frame.Position.Y.Offset, 0, math.max(0, viewport.Y - size.Y))
    frame.Position = UDim2.fromOffset(x, y)
end

local AssetFolder = "Shaman"
local AssetManifest = {
    {Name = "Circle.png", Url = "https://raw.githubusercontent.com/Rain-Design/Icons/main/Circle.png"},
    {Name = "ColorDropper.png", Url = "https://raw.githubusercontent.com/Rain-Design/Icons/main/ColorDropper.png"},
    {Name = "Close.png", Url = "https://raw.githubusercontent.com/Rain-Design/Icons/main/Close.png"},
    {Name = "CollapseArrow.png", Url = "https://raw.githubusercontent.com/Rain-Design/Icons/main/CollapseArrow.png"},
    {Name = "RadioButton.png", Url = "https://raw.githubusercontent.com/Rain-Design/Icons/main/RadioButton.png"},
    {Name = "RadioOuter.png", Url = "https://raw.githubusercontent.com/Rain-Design/Icons/main/RadioOuter.png"},
    {Name = "RadioInner.png", Url = "https://raw.githubusercontent.com/Rain-Design/Icons/main/RadioInner.png"}
}
...
local library = {
    ...
    ConfigRegistry = {},
    Theme = {
        Accent = Color3.fromRGB(48, 207, 106),
        Background = Color3.fromRGB(27, 27, 27),
        Topbar = Color3.fromRGB(25, 25, 25),
        Side = Color3.fromRGB(30, 30, 30),
        Container = Color3.fromRGB(26, 26, 26),
        Section = Color3.fromRGB(42, 42, 42),
        Stroke = Color3.fromRGB(61, 61, 61),
        SoftStroke = Color3.fromRGB(84, 84, 84),
        Text = Color3.fromRGB(217, 217, 217),
        Muted = Color3.fromRGB(191, 191, 191),
        Danger = Color3.fromRGB(217, 97, 99),
        Warning = Color3.fromRGB(194, 162, 76)
    },
    Windows = {},
    AccentRegistry = {}
}
...
function library:RegisterAccent(object, property, entry)
    if not object or not property then
        return
    end
    self.AccentRegistry[object] = {
        Property = property,
        Entry = entry
    }
end
...
function library:RefreshAccent()
    for object, info in pairs(self.AccentRegistry) do
        if object.Parent == nil and not object:IsDescendantOf(game) then
            self.AccentRegistry[object] = nil
        else
            local enabled = true
            if info.Entry and info.Entry.Enabled ~= nil then
                enabled = info.Entry.Enabled
            end
            if enabled then
                ApplyProperty(object, info.Property, self.Theme.Accent)
            end
        end
    end
end
...
function library:SaveConfig(configName)
    ...
    local AssetFolderLocal = "Shaman"
    if isfolder and makefolder and not isfolder(AssetFolderLocal) then
        makefolder(AssetFolderLocal)
    end
    local ok, encoded = pcall(function()
        return game:GetService("HttpService"):JSONEncode(self.Flags)
    end)
    if ok and encoded then
        pcall(writefile, AssetFolderLocal .. "/config_" .. tostring(configName) .. ".json", encoded)
    end
end

function library:LoadConfig(configName)
    local AssetFolderLocal = "Shaman"
    local path = AssetFolderLocal .. "/config_" .. tostring(configName) .. ".json"
    ...
    if ok and data and type(data) == "table" then
        for flag, value in pairs(data) do
            if self.ConfigRegistry[flag] then
                pcall(self.ConfigRegistry[flag], value)
            else
...
local function EnsureAssets()
    if isfolder and makefolder and not isfolder(AssetFolder) then
        makefolder(AssetFolder)
    end

    if not isfolder or not request or not writefile then
        return
    end

    local missing = {}
    for _, asset in ipairs(AssetManifest) do
        local assetPath = AssetFolder .. "/" .. asset.Name
    ...
    dMain.BackgroundColor3 = library.Theme.Background
    ...
    dTopbar.BackgroundColor3 = library.Theme.Topbar
    ...
    for _, asset in ipairs(missing) do
        local ok, response = pcall(function()
            return request({Url = asset.Url, Method = "GET"})
        end)
        if ok and response and response.Body then
            writefile(AssetFolder .. "/" .. asset.Name, response.Body)
            dText.Text = "Downloaded: " .. asset.Name
        else
            dText.Text = "Failed: " .. asset.Name
        end
    end
...
Info.Size = Info.Size or UDim2.new(0, 450, 0, 321)
Info.Position = Info.Position or UDim2.new(0.361, 0, 0.308, 0)
Info.ToggleKey = Info.ToggleKey or nil
Info.Accent = Info.Accent or self.Theme.Accent

if Info.Accent then
    self:SetTheme({Accent = Info.Accent})
end
...
shadow.Position = UDim2.new(Info.Position.X.Scale, Info.Position.X.Offset - 4, Info.Position.Y.Scale, Info.Position.Y.Offset + 6)
shadow.Size = UDim2.new(Info.Size.X.Scale, Info.Size.X.Offset + 8, 0, 0)
shadow.ZIndex = 0
shadow.Parent = shamanScreenGui
...
main.Position = Info.Position
main.Size = UDim2.new(Info.Size.X.Scale, Info.Size.X.Offset, 0, 0)
...
local dragStart
local startPos
local dragTargetPos = UDim2.new()
local prevMouseX = 0
local dragRenderConn = nil

local function StopDragSpring()
    if dragRenderConn then
        dragRenderConn:Disconnect()
        dragRenderConn = nil
    end
    Tween(main, TweenInfo.new(0.55, Enum.EasingStyle.Bounce, Enum.EasingDirection.Out), {Rotation = 0})
    Tween(shadow, TweenInfo.new(0.55, Enum.EasingStyle.Bounce, Enum.EasingDirection.Out), {Rotation = 0})
end

local function StartDragSpring()
    if dragRenderConn then return end
    dragRenderConn = RunService.RenderStepped:Connect(function()
        if Destroyed then StopDragSpring() return end
        local curX = main.Position.X.Offset
        local curY = main.Position.Y.Offset
        local newX = curX + (dragTargetPos.X.Offset - curX) * 0.2
        local newY = curY + (dragTargetPos.Y.Offset - curY) * 0.2
        main.Position = UDim2.fromOffset(newX, newY)
        shadow.Position = UDim2.fromOffset(newX - 4, newY + 6)
...
UserInputService.InputChanged:Connect(function(input)
    if dragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
        ...
        if viewport then
            ...
            rawX = math.clamp(rawX, 0, math.max(0, viewport.X - sz.X))
            rawY = math.clamp(rawY, 0, math.max(0, viewport.Y - sz.Y))
        end
        dragTargetPos = UDim2.fromOffset(rawX, rawY)
        local velX = input.Position.X - prevMouseX
        prevMouseX = input.Position.X
        local tiltTarget = math.clamp(velX * 0.3, -5, 5)
        main.Rotation = main.Rotation + (tiltTarget - main.Rotation) * 0.3
    end
end)
...
self:RegisterAccent(frame1, "BackgroundColor3")
...
closeButton.Image = getcustomasset(AssetFolder .. "/Close.png")
...
local function AnimatedClose()
    if Destroyed then return end
    closeButton.Active = false
    local cs = main.Size
    local cp = main.Position
    local cx = cp.X.Offset + cs.X.Offset * 0.5
    local cy = cp.Y.Offset + cs.Y.Offset * 0.5
    main.AnchorPoint = Vector2.new(0.5, 0.5)
    shadow.AnchorPoint = Vector2.new(0.5, 0.5)
    main.Position = UDim2.fromOffset(cx, cy)
    shadow.Position = UDim2.fromOffset(cx - 4, cy + 6)
    Tween(main, TweenInfo.new(0.12, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
        Size = UDim2.new(0, cs.X.Offset * 1.06, 0, cs.Y.Offset * 1.06)
    })
    Tween(shadow, TweenInfo.new(0.12, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
        Size = UDim2.new(0, cs.X.Offset * 1.06 + 8, 0, cs.Y.Offset * 1.06 + 8)
    })
    task.delay(0.12, function()
        Tween(main, TweenInfo.new(0.3, Enum.EasingStyle.Back, Enum.EasingDirection.In), {
            Size = UDim2.new(0, 0, 0, 0)
        })
        Tween(shadow, TweenInfo.new(0.3, Enum.EasingStyle.Back, Enum.EasingDirection.In), {
            Size = UDim2.new(0, 0, 0, 0)
        })
...
minimizeButton.MouseButton1Click:Connect(function()
    ...
    Tween(main, TweenInfo.new(.3, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Size = Opened and Info.Size or UDim2.new(Info.Size.X.Scale, Info.Size.X.Offset, 0, 30)})
end)

local editButton = Instance.new("ImageButton")
editButton.Name = "EditButton"
editButton.Image = getcustomasset(AssetFolder .. "/ColorDropper.png")
...
editButton.MouseButton1Click:Connect(function()
    ...
    if not EditOpened then
        for frame, v in pairs(ColorElements) do
            if v.Enabled then
                if frame.ClassName == "Frame" then
                TweenService:Create(frame, TweenInfo.new(.15, Enum.EasingStyle.Linear, Enum.EasingDirection.In), {BackgroundColor3 = library.Theme.Accent}):Play()
                else
                TweenService:Create(frame, TweenInfo.new(.15, Enum.EasingStyle.Linear, Enum.EasingDirection.In), {ImageColor3 = library.Theme.Accent}):Play()
                end
            end
        end
    else
...
tabContainer.BackgroundColor3 = self.Theme.Side
...
fix.Name = "Fix"
fix.BackgroundColor3 = self.Theme.Side
...
fix1.Name = "Fix"
fix1.BackgroundColor3 = self.Theme.Side
...
uIStroke.Color = library.Theme.SoftStroke
...
tabAccentBar.Position = UDim2.new(0, 0, 0.5, 0)
tabAccentBar.Size = UDim2.new(0, 2, 0, 0)
tabAccentBar.ZIndex = 3
tabAccentBar.Parent = tabFrame
local tabAccentCorner = Instance.new("UICorner")
tabAccentCorner.CornerRadius = UDim.new(0, 2)
tabAccentCorner.Parent = tabAccentBar
library:RegisterAccent(tabAccentBar, "BackgroundColor3")
...
leftContainer.BackgroundColor3 = library.Theme.Container
...
rightContainer.BackgroundColor3 = library.Theme.Container
...
uIStroke3.Color = library.Theme.Stroke
...
sectionButton.MouseButton1Click:Connect(function()
    ...
    TweenService:Create(sectionIcon, TweenInfo.new(.25, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Rotation = Closed.Value and 45 or 0}):Play()
end)
...
function insidekeybind:Set(keycode)
...
function insidekeybind:Get()
    return PressKey
end

return insidekeybind
...
textButton.MouseButton1Down:Connect(function()
    ...
    ripple.Position = UDim2.fromOffset(mp.X - ap.X, mp.Y - ap.Y)
...
function buttontable:SetText(text)
    buttonText.Text = text or buttonText.Text
end

function buttontable:Fire()
    task.spawn(function()
        pcall(Info.Callback)
    end)
end
...
function insideinput:Set(text)
    inputTextBox.Text = text or ""
    if Info.Flag ~= nil then
        library.Flags[Info.Flag] = inputTextBox.Text
    end
end

function insideinput:Get()
    return inputTextBox.Text
end

function insideinput:SetPlaceholder(text)
    inputTextBox.PlaceholderText = text or inputTextBox.PlaceholderText
end
...
library:RegisterAccent(toggleFrame, "BackgroundColor3", ColorElements[toggleFrame])
...
circleIcon.Image = getcustomasset(AssetFolder .. "/Circle.png")
...
function insidetoggle:Set(bool)
    ...
    TweenService:Create(circleIcon, TweenInfo.new(.25, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Position = Toggled and UDim2.new(0, 16,0.067, 0) or UDim2.new(0, 1,0.067, 0)}):Play()
    if not Toggled then
        TweenService:Create(toggleFrame, TweenInfo.new(.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {BackgroundColor3 = Color3.fromRGB(68, 68, 68)}):Play()
    elseif Toggled and not EditOpened then
        TweenService:Create(toggleFrame, TweenInfo.new(.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {BackgroundColor3 = library.Theme.Accent}):Play()
    end
...
function insidetoggle:SetText(text)
    toggleText.Text = text or toggleText.Text
end

if Info.Flag ~= nil then
    library.ConfigRegistry[Info.Flag] = function(v) insidetoggle:Set(v == true or v == "true") end
end
...
innerSlider.BackgroundColor3 = library.Theme.Accent
...
library:RegisterAccent(innerSlider, "BackgroundColor3", ColorElements[innerSlider])
...
function insideslider:Set(value)
    local clamped = math.clamp(value, Info.Minimum, Info.Maximum)
    local scale = (clamped - Info.Minimum) / (Info.Maximum - Info.Minimum)
    innerSlider.Size = UDim2.new(scale, 0, 0, 4)
    sliderValueText.Text = tostring(clamped) .. Info.Postfix
    if Info.Flag ~= nil then
        library.Flags[Info.Flag] = clamped
    end
    pcall(Info.Callback, clamped)
end

function insideslider:Get()
    if Info.Flag ~= nil then
        return library.Flags[Info.Flag]
    end
    local numericText = sliderValueText.Text:gsub(Info.Postfix, "")
    return tonumber(numericText)
end

function insideslider:SetText(text)
    sliderText.Text = text or sliderText.Text
end

if Info.Flag ~= nil then
    library.ConfigRegistry[Info.Flag] = function(v)
        local num = tonumber(v)
        if num then insideslider:Set(num) end
    end
end
...
dropdownIcon.Image = getcustomasset(AssetFolder .. "/CollapseArrow.png")
...
function insidedropdown:SetText(text)
    dropdownText.Text = text or dropdownText.Text
end
...
radioButtonIcon.Image = getcustomasset(AssetFolder .. "/CollapseArrow.png")
...
radioButtonIcon2.Image = getcustomasset(AssetFolder .. "/RadioButton.png")
...
radioOuter.Image = getcustomasset(AssetFolder .. "/RadioOuter.png")
...
radioInner.Image = getcustomasset(AssetFolder .. "/RadioInner.png")
...
library:RegisterAccent(radioInner, "ImageColor3", ColorElements[radioInner])
library:RegisterAccent(radioOuter, "ImageColor3", ColorElements[radioOuter])
...
radioTextButton.MouseButton1Click:Connect(function()
    ...
    if not EditOpened then
        TweenService:Create(radioInner, TweenInfo.new(.15, Enum.EasingStyle.Linear, Enum.EasingDirection.In), {ImageColor3 = RadioOpened and library.Theme.Accent or Color3.fromRGB(191, 191, 191)}):Play()
        TweenService:Create(radioOuter, TweenInfo.new(.15, Enum.EasingStyle.Linear, Enum.EasingDirection.In), {ImageColor3 = RadioOpened and library.Theme.Accent or Color3.fromRGB(191, 191, 191)}):Play()
    end
end)
...
function insideradio:Set(value)
    for _, child in pairs(radioContainer:GetChildren()) do
        if child.ClassName == "Frame" and child:FindFirstChild("RadioText") and child.RadioText.Text == value then
            RadioSelected = child
            if Info.Flag ~= nil then
                library.Flags[Info.Flag] = value
			end
            pcall(Info.Callback, value)
            for _, other in pairs(radioContainer:GetChildren()) do
                if other.ClassName == "Frame" and other:FindFirstChild("RadioOuter") then
                    local active = other == child
                    ColorElements[other.RadioOuter].Enabled = active
                    ColorElements[other.RadioOuter.RadioInner].Enabled = active
                    other.RadioText.TextColor3 = active and Color3.fromRGB(255, 255, 255) or library.Theme.Muted
                    other.RadioOuter.ImageColor3 = active and library.Theme.Accent or library.Theme.Muted
                    other.RadioOuter.RadioInner.ImageColor3 = active and library.Theme.Accent or library.Theme.Muted
                end
            end
            break
        end
...
function insideradio:Get()
    if Info.Flag ~= nil then
        return library.Flags[Info.Flag]
    end
    return RadioSelected and RadioSelected.RadioText.Text or nil
end
...
tabTextButton.MouseButton1Click:Connect(function()
    ...
    for _,v in pairs(scrollingContainer:GetChildren()) do
        if v ~= tabButton and v.Name == "TabButton" then
            TweenService:Create(v.TabFrame, TweenInfo.new(.2, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {BackgroundTransparency = .96}):Play()
            local bar = v.TabFrame:FindFirstChild("TabAccentBar")
            if bar then Tween(bar, TweenInfo.new(0.2, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {Size = UDim2.new(0, 2, 0, 0)}) end
        end
    end
    TweenService:Create(tabFrame, TweenInfo.new(.2, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {BackgroundTransparency = .82}):Play()
    Tween(tabAccentBar, TweenInfo.new(0.3, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Size = UDim2.new(0, 2, 0.8, 0)})
...
function window:SetTitle(text)
    textLabel.Text = text or Info.Text
end
...
uIStroke2.Color = self.Theme.Stroke
...
task.delay(0, function()
    if not Destroyed then
        ...
        Tween(shadow, TweenInfo.new(0.45, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Size = UDim2.new(Info.Size.X.Scale, Info.Size.X.Offset + 8, Info.Size.Y.Scale, Info.Size.Y.Offset + 8)})
    end
end)
...
function library:Notify(Info)
    ...
    Info.Duration = Info.Duration or 4
    Info.Color    = Info.Color    or self.Theme.Accent

    if not _notifGui or not _notifGui.Parent then
        _notifGui = Instance.new("ScreenGui")
        _notifGui.Name = "ShamanNotifications"
        ...
        holder.Size = UDim2.new(0, 260, 1, -28)
        holder.Parent = _notifGui

        local ull = Instance.new("UIListLayout")
        ull.SortOrder = Enum.SortOrder.LayoutOrder
        ull.VerticalAlignment = Enum.VerticalAlignment.Bottom
        ull.Padding = UDim.new(0, 7)
        ull.Parent = holder
    end

    local holder = _notifGui.Holder
    ...
    toast.Position = UDim2.new(1.6, 0, 0, 0)
    toast.LayoutOrder = _notifCount
    toast.Parent = holder
    ...
    bodyLbl.TextColor3 = Color3.fromRGB(175, 175, 175)
    bodyLbl.TextSize = 11
    bodyLbl.TextXAlignment = Enum.TextXAlignment.Left
    bodyLbl.TextWrapped = true
    bodyLbl.BackgroundTransparency = 1
    bodyLbl.Position = UDim2.new(0, 14, 0, 29)
    ...
    progBar.BackgroundColor3 = Info.Color
